...
#include <franka_advanced_controllers/compliance_joint_paramConfig.h>
...
class JointImpedanceAdvancedController : public controller_interface::MultiInterfaceController<
 ...
 private:
  ...
  // Dynamic reconfigure
  std::unique_ptr<dynamic_reconfigure::Server<franka_advanced_controllers::compliance_joint_paramConfig>>
      dynamic_server_compliance_joint_param_;

  ros::NodeHandle dynamic_reconfigure_compliance_joint_param_node_;
  void complianceJointParamCallback(franka_advanced_controllers::compliance_joint_paramConfig& config,
                               uint32_t level);
